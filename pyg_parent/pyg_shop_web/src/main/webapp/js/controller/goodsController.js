...
app.controller('goodsController' ,function($scope,$location,$controller,goodsService,uploadService,itemCatService,typeTemplateService){
        ...
        if(object!=null){
			if($event.target.checked){
                ...
                if(object.attributeValue.length==0){
                    ...
                    //删除整个对象
                    // if($scope.isSpecEmpty(name,value)){
                    // 	//alert(JSON.stringify( $scope.entity.itemList));
                    //     $scope.entity.itemList.splice(0,1);
                    // }
                }
    ...
    $scope.isSpecEmpty=function (name,value) {
        var spec = $scope.entity.itemList[0].spec;
       // alert("aa")
        //alert('{'+'"'+name+'"'+':'+'"'+value+'"'+'}')
       // alert(JSON.stringify(spec))
       if ( JSON.stringify(spec)=='{'+'"'+name+'"'+':'+'"'+value+'"'+'}'){
          // alert("bbb")
            return true;
       }
        // for(var key in spec) {
    ...
    $scope.createItemList=function ($event) {
      //删除整个对象的条件
        // if($event.target.checked){
        //     //列表初始化
...
//加载商品分类列表
    ...
    //根据规格名称和选项名称返回是否被勾选 specName:属性名  optionName：属性值
    $scope.checkAttributeValue=function (specName, optionName) {
        var items= $scope.entity.goodsDesc.specificationItems;
        var object= $scope.searchObjectByKey(items,'attributeName',specName);
        if(object==null){
            return false;
        }else if(object.attributeValue.indexOf(optionName)>=0){
            return true;
        }else {
            return false;
        }
